package com.steepgraph.ta.framework.V6R2013x.pages;
...
public class Library extends MasterLibrary {
	...
	public void globalSearch(Driver driver, Map<String, String> attributeMap, String strSearchInputText)
		...
		// else {
		// WebElement wbElementGlobalSearchSubmit =
		// driver.findElement(By.xpath("//div[@id='pageHeadDiv']//img[@name='AEFGlobalFullTextSearch']"));
	...
	private WebElement selectDialogItem(Driver driver, String[] inputArray) throws Exception {
		...
		if (length == 1)
			return driver
					.findElement(By.xpath("//div[@class='mmenu dialog']//li/span[text()='" + inputArray[0] + "']/.."));

		for (int i = 0; i < length; i++) {
			if (i == 0) {
				wbPreviousCommandList = driver.findElements(
						By.xpath("//div[@class='mmenu dialog']//h3/span[text()='" + inputArray[i] + "']/.."));
			} else if (i == 1) {
				for (Iterator<?> iterator = wbPreviousCommandList.iterator(); iterator.hasNext();) {
	...
	public void clickCategoryCommand(Driver driver, Map<String, String> attributeMap) throws Exception {
		...
		WebElement wCatergoryCommand = driver
				.findElement(By.xpath("//div[@class='mmenu-content']//li//span[text()='" + strCategoryName + "']/.."));
	...
	public void openActionToolbarMenu(Driver driver, Map<String, String> attributeMap) throws Exception {
		...
		if ("true".equalsIgnoreCase(strHasTVC)) {
			try {
				weActionsMenu = driver
						.findElement(By.xpath("//div[contains(@id,'toolbar')]//span[text()='Actions']/.."));
		...
		for (int i = 0; i < labelArray.length; i++) {
			...
			if (!isActualTVCMenu)
				wbCommand = driver.findElement(
						By.xpath("//div[@class='mmenu page' and contains(@style,'display: block;')]//li//span[text()='"
								+ label + "']/.."));
			else
				wbCommand = driver
						.findElement(By.xpath("//div[@class='drop-content']//span[text()='" + label + "']/.."));
